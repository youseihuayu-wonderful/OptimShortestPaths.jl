...
AUROC~0.97. Our single-objective AUROC of 0.77 is substantially lower.
However, the comparison is not direct: DWPC uses a logistic regression
over 1,206 metapath-based features (all path types up to length~4,
weighted by node degree), while our method uses a single shortest-path
distance per dimension. DWPC effectively learns an optimal linear
combination of all topological features, making it a supervised method,
whereas our framework is unsupervised (no training on the evaluation
labels). Furthermore, our method provides a single interpretable path
per prediction, while DWPC provides a feature importance vector across
metapaths. The approaches are complementary: DWPC optimizes prediction
accuracy, while our framework optimizes interpretability and
